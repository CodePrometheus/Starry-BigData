 ...
 **/
public class CustomWindowImpl {

    public static void main(String[] args) throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment()
                .setParallelism(1).setRuntimeMode(STREAMING);
        ...
        SingleOutputStreamOperator<Tuple2<EventBean, Integer>> beanStream = source.map(s -> {
            ...
            return Tuple2.of(bean, 1);
                ...
                .withTimestampAssigner((SerializableTimestampAssigner<Tuple2<EventBean, Integer>>) (element, recordTimestamp) ->
                        element.f0.getTimeStamp()));


        OutputTag<Tuple2<EventBean, Integer>> lateDataOutputTag = new OutputTag<>("late_data",
                TypeInformation.of(new TypeHint<Tuple2<EventBean, Integer>>() {
                }));
        ...
        SingleOutputStreamOperator<String> sumResult = beanStream.keyBy(tp -> tp.f0.getGuid())
                ...
                .evictor(MyTimeEvictor.of(Time.seconds(10))).apply((WindowFunction<Tuple2<EventBean, Integer>, String, Long, TimeWindow>)
                        (aLong, window, input, out) -> {
                            int count = 0;
                            for (Tuple2<EventBean, Integer> eventBean2IntegerTuple2 : input) {
                                count++;
                            }
